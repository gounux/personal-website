---
icon: material/account-hard-hat-outline
---

# GIS

:ninja: Currently working as a geo-ninja with [OPENGIS.ch](https://www.opengis.ch/), pushing the [QField](https://qfield.org/) and [QFieldCloud](https://app.qfield.cloud) world further.

:earth_africa: I'm an enthusiast and sometimes moving to sub-Saharan Africa, working with infrastructure providers, mapping some stuff.

:speech_balloon: Blogging and doing some geostuff with [Geotribu](https://geotribu.fr/), a spontaneous group of french-speaking FOSS geo-enthusiasts.

:classical_building: Volunteering as communication guy with [OSGeo-fr](https://www.osgeo.fr/), yearly organizing [the French-Speaking QGIS Users Meeting](https://conf.qgis.osgeo.fr/). 